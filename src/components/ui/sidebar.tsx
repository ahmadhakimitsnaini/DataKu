...
const SIDEBAR_COOKIE_MAX_AGE = 60 * 60 * 24 * 7; // Simpan state sidebar selama 7 hari
const SIDEBAR_WIDTH = "16rem";
const SIDEBAR_WIDTH_MOBILE = "18rem";
const SIDEBAR_WIDTH_ICON = "3rem";
const SIDEBAR_KEYBOARD_SHORTCUT = "b"; // Shortcut Cmd+B atau Ctrl+B
...
// Hook untuk mengakses state sidebar dari dalam komponen menu
function useSidebar() {
...
// Provider: Mengelola state global sidebar (terbuka/tertutup) dan menyimpannya di cookie
...
// Sidebar: Komponen pembungkus area sidebar (menyesuaikan versi desktop atau mobile sheet)
...
// Trigger: Tombol ikon menu (hamburger/panel) untuk membuka/menutup sidebar
...
// Rail: Area tipis di pinggir sidebar yang bisa diklik untuk membuka sidebar
const SidebarRail = React.forwardRef<HTMLButtonElement, React.ComponentProps<"button">>(({ className, ...props }, ref) => {
  const { toggleSidebar } = useSidebar();
  return (
    <button ref={ref} data-sidebar="rail" aria-label="Toggle Sidebar" tabIndex={-1} onClick={toggleSidebar} className={cn("absolute inset-y-0 z-20 hidden w-4 -translate-x-1/2 transition-all ease-linear after:absolute after:inset-y-0 after:left-1/2 after:w-[2px] group-data-[side=left]:-right-4 group-data-[side=right]:left-0 hover:after:bg-sidebar-border sm:flex", "[[data-side=left]_&]:cursor-w-resize [[data-side=right]_&]:cursor-e-resize", "[[data-side=left][data-state=collapsed]_&]:cursor-e-resize [[data-side=right][data-state=collapsed]_&]:cursor-w-resize", "group-data-[collapsible=offcanvas]:translate-x-0 group-data-[collapsible=offcanvas]:after:left-full group-data-[collapsible=offcanvas]:hover:bg-sidebar", "[[data-side=left][data-collapsible=offcanvas]_&]:-right-2", "[[data-side=right][data-collapsible=offcanvas]_&]:-left-2", className)} {...props} />
  );
});
SidebarRail.displayName = "SidebarRail";

// Inset: Kontainer pembungkus halaman utama (di luar sidebar)
...
// Input: Form input khusus di dalam sidebar (misal untuk fitur pencarian menu)
...
// Header, Content, Footer: Area pembagian vertikal di dalam Sidebar
...
// Group & Label: Untuk memisahkan menu berdasarkan kategori (misal: "Settings", "Projects")
...
// Menu & Items: Wrapper untuk list link navigasi
...
// Skeleton untuk loading state menu
