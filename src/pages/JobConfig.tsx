  ...
  return (
    <div className="min-h-screen bg-background relative">
      ...
      <nav className="relative z-10 flex items-center justify-between px-6 lg:px-12 py-5 border-b border-border max-w-[1600px] mx-auto">
        ...
        <Link to={`/project/${id}`}><Button variant="ghost">Back to Profiling</Button></Link>
      </nav>

      <main className="relative z-10 max-w-4xl mx-auto px-6 py-12">
        <div className="mb-8">
          <h1 className="text-3xl font-bold mb-2">Configure Training Job</h1>
          <p className="text-muted-foreground">Select the target column you want to predict.</p>
        </div>
        ...
        <div className="flex justify-end gap-4">
          <Link to={`/project/${id}`}><Button variant="outline" size="lg">Cancel</Button></Link>
          <Button size="lg" className="gap-2" disabled={!selectedTarget} onClick={() => navigate(`/project/${id}/models`)}>
