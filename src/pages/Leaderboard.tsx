      ...
      <main className="relative z-10 max-w-5xl mx-auto px-6 py-12">
        <div className="flex items-start justify-between mb-8">
          <div>
            ...
            <p className="text-muted-foreground flex items-center gap-2">
              <Activity className="w-4 h-4" /> Training in progress... (Project: {id})
            </p>
          </div>
        ...
        <div className="space-y-4">
          {sortedModels.map((model, index) => {
            ...
            return (
              <div key={model.id} className={`glass rounded-xl p-6 transition-colors ${isBest ? "border-primary/50 bg-primary/5" : "hover:border-primary/20"}`}>
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-4">
                    <div className="w-12 h-12 rounded-full bg-secondary flex items-center justify-center font-bold text-lg">#{index + 1}</div>
                    <div>
                  ...
                  {model.status === "completed" ? (
                    <div className="flex items-center gap-12 text-right">
                      ...
                      <Button size="sm" className="gap-2 min-w-[140px]" variant={isBest ? "default" : "secondary"}>
                        <ExternalLink className="w-4 h-4" /> Deploy API
                      </Button>
                    </div>
                  ) : (
                    <div className="flex-1 max-w-md ml-12">
                      <div className="text-xs text-muted-foreground mb-2">Training...</div>
